...
\code{c02_basic_values}{python}{
    print(4)
    print(3.5)
    print("Studio")
    print(True)
    print(None)
}{Five basic Python values}
...
\code{c02_type_check}{python}{
    print(type(4))
    print(type(3.5))
    print(type("Studio"))
}{Inspect three value types}
...
\code{c02_first_variables}{python}{
    room_name = "Studio"
    width_m = 6.0
    length_m = 8.0
    ...
    print(width_m)
    print(length_m)
}{Assign and display three variables}
...
\code{c02_reassignment_output}{text}{
    3
    4
...
\code{c02_precedence}{python}{
    result_a = 4 + 3 * 2
    result_b = (4 + 3) * 2

    print(result_a)
    print(result_b)
}{Parentheses change calculation order}

Expected output:

\code{c02_precedence_output}{text}{
    10
    14
}{Results with and without parentheses}
...
\code{c02_first_fstring}{python}{
    room_name = "Studio"
    area_m2 = 48.0

    print(f"{room_name} area: {area_m2} m2")
}{Insert values into an f-string}

Use a format specification to control decimal places.

\code{c02_decimal_format}{python}{
    width_m = 5.75
    length_m = 7.2
    area_m2 = width_m * length_m
...
\code{c02_comments}{python}{
    ...
    area_m2 = width_m * length_m
    print(f"Area: {area_m2:.2f} m2")
}{A comment records an important assumption}
...
\code{c02_name_error}{python}{
    print(area_m2)
}{This intentionally produces a NameError}

Assign \c{area_m2} before trying to use it.

\hhh{Mixing text and numbers with plus}

\code{c02_type_error}{python}{
    area_m2 = 48.0
    print("Area: " + area_m2)
}{This intentionally produces a TypeError}

Use an f-string instead:

\code{c02_type_error_fixed}{python}{
    area_m2 = 48.0
    print(f"Area: {area_m2} m2")
}{Use an f-string to combine text and a number}

\hhh{Using an unclear name}

\code{c02_unclear_names}{python}{
    a = 6.0
    b = 8.0
    c = a * b
}{Valid code with unclear names}
...
\code{c02_practice_level0}{python}{
    width_m = 4.0
    length_m = 6.0
    area_m2 = width_m * length_m
    print(area_m2)
...
Complete the missing expressions so the program calculates perimeter.

\code{c02_practice_level2}{python}{
    width_m = 5.0
    length_m = 9.0
    perimeter_m = 2 * (width_m + length_m)

    print(f"Perimeter: {perimeter_m:.1f} m")
}{A completed perimeter calculation}
...
\code{c02_solution_level3}{python}{
    width_m = float(input("Width in meters: "))
    length_m = float(input("Length in meters: "))
