...
\booklink{ch:17}{Chapter 17} formulated an optimization model and handed it to a
...
\booklink{ch:17}{Chapter 17}: decision variables, objective, bounds, and
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch19_search_methods.pdf}
...
\begin{lstlisting}[language=text, style=block, caption={Descent from a poor starting design}, label={c19_descent_output}, captionpos=b]
Steps taken: 22
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch19_gradient_descent.pdf}
...
\begin{lstlisting}[language=text, style=block, caption={The same optimum at different evaluation costs}, label={c19_lbfgs_output}, captionpos=b]
...
\booklink{ch:17}{Chapter 17}: \c{x} is the recommendation, \c{fun} its cost,
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch19_metaheuristic_search.pdf}
...
\begin{lstlisting}[language=text, style=block, caption={Library global optimizers find the same design at different costs}, label={c19_scipy_output}, captionpos=b]
...
The runnable program \c{examples/c19_facade_optimization.py} combines the
...
In Listing~\ref{c19_descent}, identify the line that enforces the bounds, the
...
\booklink{ch:20}{Chapter 20} applies the same comparison discipline to
