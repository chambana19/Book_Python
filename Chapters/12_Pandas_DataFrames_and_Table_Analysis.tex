...
files, Boolean conditions, functions, and the NumPy ideas of vectorized
calculation and missing numerical values.
\end{NoteBox}
...
\code{c12_groupby_summary}{python}{
    ...
    area_by_zone = (
        rooms.groupby("zone", as_index=False)["area_m2"]
        .agg(["count", "sum", "mean"])
    )
...
Read the chain from left to right: group by \c{zone}, select \c{area_m2}, then
calculate three summaries. Short line breaks make each step visible.
...
\textbf{Hints}
\begin{itemize}
    \item Use one integer floor value per row.
    \item Group with \c{groupby("floor", as_index=False)}.
    \item Named aggregation can create clear output column names.
\end{itemize}
...
\hh{Part IV checkpoint continued}
...
\begin{tcolorbox}[title=Chapter 12 summary,colframe=orange,colback=white,arc=2mm]
