...
\c{suffix}, and \c{parent} (\c{.parent} gives the folder that contains the
path). Predict the results first. Then create only the parent folder with
\c{mkdir(exist_ok=True)} and check \c{exists()} for both the folder and the
still-unwritten file.
\end{TryBox}
...
The sequence "\textbackslash n" represents a line break.
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Write text with a with block},
...
suffix (the stem is the filename without its suffix).
