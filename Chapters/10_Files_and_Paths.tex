...
}{Create a study data folder safely}
...
}{Write three lines of UTF-8 text}
...
\code{c10_read_text}{python}{
    ...
    print(content)
}{Create and read a small text file}
...
\code{c10_with_open_write}{python}{
    from pathlib import Path
...
}{Write text with a context manager}
...
}{Append one line to an existing file}
...
\code{c10_line_iteration}{python}{
    from pathlib import Path
...
}{Split two fields on a controlled delimiter}
...
}{Choose based on whether an optional file exists}
...
}{Write rows with the csv module}
...
\code{c10_file_functions}{python}{
    from pathlib import Path
    ...
    print(room_names)
}{Separate saving and loading into functions}
...
\code{c10_practice_level0}{python}{
    from pathlib import Path

    path = Path("study_data") / "notes.txt"
    print(path.name)
    print(path.suffix)
}{Predict two path properties}
...
Create a folder, write three lines, read them back, and display the text.

\code{c10_practice_level2}{python}{
    from pathlib import Path
...
\textbf{Level 2:} The three material names appear on separate lines.

\textbf{Level 3}

\code{c10_solution_level3}{python}{
    import csv
    from pathlib import Path
...
\code{c10_solution_level4}{python}{
    import json
    from pathlib import Path
