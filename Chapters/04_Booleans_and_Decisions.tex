...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Store and display boolean values},
    label={c04_boolean_values},
    captionpos=b
]
is_accessible = True
needs_review = False
...
\begin{ExplainBox}{A boolean expression is a proposition}
A proposition is a statement that can be evaluated as true or false. The
expression \c{area_m2 >= 20} proposes that the current area is at least 20;
Python evaluates the current value and returns one boolean. This is different
from assignment: \c{area_m2 = 20} changes which value the name refers to,
whereas \c{area_m2 == 20} asks a question without changing it.
\end{ExplainBox}

Name stored booleans so they read as yes-or-no questions: \c{is_accessible},
\c{has_daylight}, \c{needs_review}, or \c{can_exit}. Such names preserve the
meaning when they later appear in \c{if is_accessible:}. A vague name such as
\c{status} might instead contain text, a number, or a boolean and forces the
reader to inspect other code.

Not every question should be tested with exact equality. Exact equality is
appropriate for integers, strings, categories, and deliberate sentinel values.
Measured floats may require a tolerance because Chapter 2 showed that their
binary representations can be approximate.
...
\begin{SyntaxBox}{Comparison produces a new boolean value}
In \c{area_m2 >= 20.0}, Python evaluates the left and right operands with the
\c{>=} operator. The result is one value---\c{True} or \c{False}---which can be
stored, printed, or used directly by an \c{if} statement.
\end{SyntaxBox}
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Check whether a value is inside a range},
    label={c04_chained_comparison},
    captionpos=b
]
area_m2 = 32.0
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Require two conditions},
    label={c04_and_operator},
    captionpos=b
]
width_m = 4.0
length_m = 7.0
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Accept either of two values},
    label={c04_or_operator},
    captionpos=b
]
room_type = "Studio"
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Reverse a boolean value},
    label={c04_not_operator},
    captionpos=b
]
needs_review = False
...
\bookfigure[0.84\textwidth]{Figures/Boolean/Boolean_operation.pdf}
{Boolean operators combine or reverse yes--no conditions.}
{fig:ch04-boolean-operations}

\begin{TryBox}{Build a four-row truth table}
Set \c{has_daylight} and \c{has_access} to every possible pair of boolean
values. For each pair, print the results of \c{has_daylight and has_access}
and \c{has_daylight or has_access}. Use the output to explain the difference
between ``both requirements'' and ``at least one requirement.''
\end{TryBox}
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Check whether a value is in a list},
    label={c04_membership_condition},
    captionpos=b
]
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Check that a value is absent},
    label={c04_not_in},
    captionpos=b
]
allowed_types = ["Studio", "Office", "Classroom"]
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Choose one of two branches},
    label={c04_if_else},
    captionpos=b
]
area_m2 = 18.0
...
\bookfigure[0.82\textwidth]{Figures/Generated/ch04_decision_trace.pdf}
{A two-branch conditional evaluates one Boolean condition, follows exactly one branch, and then continues after the statement.}
{fig:ch04-decision-trace}

When tracing a decision, write the condition's Boolean result before choosing
a branch. This prevents indentation or variable names from distracting you
from the control-flow rule.
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Classify a value with three branches},
    label={c04_if_elif_else},
    captionpos=b
]
area_m2 = 32.0
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Trace named conditions before the decision},
    label={c04_trace_decision},
    captionpos=b
]
width_m = 4.0
length_m = 6.0
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Use a string directly as a condition},
    label={c04_truthiness},
    captionpos=b
]
room_name = "Studio"
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={A two-level nested decision},
    label={c04_nested_if},
    captionpos=b
]
area_m2 = 32.0
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Predict two boolean results},
    label={c04_practice_level0},
    captionpos=b
]
width_m = 3.0
print(width_m > 2.5)
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Test several values against one range},
    label={c04_practice_level1},
    captionpos=b
]
area_m2 = 25.0
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={Complete a two-condition decision},
    label={c04_practice_level2},
    captionpos=b
]
width_m = 4.0
length_m = 7.0
