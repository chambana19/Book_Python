...
reports no problem, and decide whether that is the right behaviour for a study
...
room-hour". Chapter~\ref{ch:12}'s grouping operations exist largely to move
...
\begin{lstlisting}[language=text, style=block, caption={Height, width, and three colour channels}, label={c14_image_output}, captionpos=b]
shape: (48, 64, 3) dtype: uint8
one pixel: [235 172  98]
\end{lstlisting}

\begin{SyntaxBox}{Reading an image's shape}
\c{(48, 64, 3)} is 48 rows of pixels, 64 columns, and 3 colour channels. Two
...
\c{grayscale.mean()} averages the three colour channels into one intensity per
...
JSON & nested records, APIs & verbose, no types, whole-file reads \\
...
\begin{enumerate}
    ...
    \item 48 pixel rows, 64 pixel columns, and 3 colour channels: height first,
          then width.
