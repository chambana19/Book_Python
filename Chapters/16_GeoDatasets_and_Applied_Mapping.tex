...
% Chapter 16: geodatasets and applied mapping
...
\begin{NoteBox}
\textbf{Prerequisites.} Complete \booklink{ch:15}{Chapter 15} first. You should
...
\begin{NoteBox}
\c{geodatasets} was already installed alongside GeoPandas in
\booklink{ch:15}{Chapter 15}; there is nothing new to install here.
\end{NoteBox}
...
A runnable copy is provided in \c{examples/c16_chicago_mapping.py}.

\begin{lstlisting}[
    language=python,
    style=block,
    caption={Create a Chicago population and grocery-location analysis},
    label={c16_chicago_application},
    captionpos=b
]
...
and the modifiable areal unit problem -- the same underlying data can look
very different when it is aggregated into different-sized or
differently-drawn boundaries -- before making policy claims.
...
Identify which four lines in Listing~\ref{c16_get_path} perform discovery,
