% Part VI -- Geospatial Data and Applied Mapping
% Chapter 15: GeoPandas foundations
...
GeoPandas and geodatasets are not included with Spyder's standalone
installer. If you are using Spyder, follow
\booklink{note:spyder-add-package}{the three steps from Chapter 1} once for
each package name: \c{geopandas}, then \c{geodatasets}. If you are using
Python 3 and VS Code, install the mapping libraries in a terminal:

\begin{lstlisting}[
    language=bash,
    style=block,
    caption={Install the mapping libraries from a terminal},
    label={c15_install_geopandas},
    captionpos=b
]
...
\begin{enumerate}
    ...
    \item The assignment stores the result -- attributes and geometry together -- in \c{points}.
\end{enumerate}
...
A runnable copy is provided in \c{examples/c15_site_service_areas.py}.

\begin{lstlisting}[
    language=python,
    style=block,
    caption={Build and export a site-service map},
    label={c15_site_service_application},
    captionpos=b
]
...
\bookfigure[0.86\textwidth]{Figures/Generated/c15_site_buffers.pdf}
{Rendered output from Listing~\ref{c15_site_service_application}. The projected axes make the 400-meter radius meaningful.}
{fig:ch15-site-buffers}
...
Continuing with the \c{points} and \c{service_areas} GeoDataFrames from the
applied example above (both now in the projected CRS, with
\c{service_areas} carrying a \c{"name"} column):

\begin{lstlisting}[
    language=python,
    style=block,
    caption={Test simple geometric relationships},
    label={c15_predicates},
    captionpos=b
]
...
In Listing~\ref{c15_create_points}, identify the attribute columns, geometry
