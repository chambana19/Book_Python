...
\booklink{ch:19}{Chapter 19} fitted one regression model and evaluated it once.
...
optimization problem in the sense of \booklink{ch:20}{Chapter 20}, with one
...
plotting, and the supervised-learning vocabulary of \booklink{ch:19}{Chapter 19}:
...
\begin{lstlisting}[language=text, style=block, caption={An imbalanced target}, label={c21_data_output}, captionpos=b]
Rows: 900
...
\begin{lstlisting}[language=text, style=block, caption={Both parts keep the same complaint rate}, label={c21_split_output}, captionpos=b]
...
\begin{lstlisting}[language=text, style=block, caption={High accuracy, zero usefulness}, label={c21_baseline_output}, captionpos=b]
...
F1 & One number balancing the two & $2 \cdot \frac{P \times R}{P+R}$ \\
\end{tabular}
\end{center}
...
data, exactly the error described in \booklink{ch:19}{Chapter 19}. A
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch21_classification_workflow.pdf}
...
\begin{lstlisting}[language=text, style=block, caption={Cross-validated comparison on training data only}, label={c21_crossval_output}, captionpos=b]
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch21_model_selection.pdf}
...
average is more stable. This is why the forest scored 0.836 where the
depth-5 tree scored 0.741.
...
\begin{lstlisting}[language=text, style=block, caption={Twelve combinations, five folds, one winner}, label={c21_gridsearch_output}, captionpos=b]
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch21_threshold_tradeoff.pdf}
...
\begin{lstlisting}[language=text, style=block, caption={One model, five classifiers}, label={c21_threshold_output}, captionpos=b]
...
\begin{lstlisting}[language=text, style=block, caption={Measured reliance, ranked}, label={c21_importance_output}, captionpos=b]
...
Compare this with the answer key from Listing~\ref{c21_data}. The three main
...
\label{sec:c21-application}

The runnable program \c{examples/c21_comfort_complaint_classification.py}
...
\textbf{Level 1:} With \c{scoring="accuracy"} the same five folds give logistic
pipeline 0.797 +/- 0.021, decision tree (depth 5) 0.781 +/- 0.014, and random
forest 0.825 +/- 0.026 -- the same ranking as ROC-AUC, but compressed into a
narrow band just above the 0.76 majority-class baseline. Accuracy is measured
at a fixed 0.5 threshold and counts every correct "no complaint" the same as a
correct "complaint," so on this 76/24 split it is dominated by the majority
class and barely separates a genuinely better ranking model from one that
mostly agrees with the baseline. ROC-AUC is the comparison to trust for this
problem: it summarizes ranking quality across every threshold instead of one,
which is exactly why the chapter chose it in the first place.
...
\booklink{ch:22}{Chapter 22} adds one more candidate to this comparison -- a
