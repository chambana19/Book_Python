...
\begin{itemize}
    ...
    \item explain local minima, model sensitivity, and the resolution limits of a grid search; and
...
the function returns, typically by adding weighted or penalized terms.
\booklink{ch:21}{Chapter 21} leans on the same requirement from the other
direction: a cross-validation score reduces many individual predictions to
the one comparable number a model comparison needs.
\end{ExplainBox}
...
This table describes the model's structure. Figure~\ref{fig:ch18-model}
describes the workflow that uses it -- formulate the model above, then choose
a search algorithm and independently verify the result it returns; "search
algorithm" there is how the model gets solved, not a sixth structural piece.
...
interface. If you are using Spyder, it is already included -- skip ahead to
the import below. If you are using Python 3 and VS Code, install it once in
a terminal:

\begin{lstlisting}[
    language=bash,
    style=block,
    caption={Install SciPy from a terminal, not inside a Python file},
    label={c18_install_scipy},
    captionpos=b
]
...
print(result_2d.x, result_2d.fun, result_2d.success)
\end{lstlisting}

\c{x0} is the initial candidate vector. The objective unpacks that vector into
named decisions and still returns one scalar. The \c{"L-BFGS-B"} method uses
gradient information, which SciPy estimates automatically here;
\booklink{ch:20}{Chapter 20} opens up what a gradient is and how such solvers
use it. Multivariable problems may have several local minima, stronger
interactions, and greater sensitivity to initial conditions; visualize slices
and try justified starting points.
...
\begin{enumerate}
    \item What are the five components shown in Figure~\ref{fig:ch18-model}?
