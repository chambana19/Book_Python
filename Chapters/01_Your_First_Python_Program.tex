...
\begin{enumerate}
    ...
    \item Download and open the current Python Install Manager.
    \item Choose \textbf{Install}, then open a new terminal.
    \item Enter \texttt{python}. If prompted, allow the manager to install the
...
\begin{enumerate}
    ...
    \item In the new \texttt{Python 3.x} folder in Applications, run
          \texttt{Install Certificates.command}.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Command not found}: open a new terminal and try \texttt{py} or
          \texttt{python3}. On Windows, also check that the Python app execution
          aliases are enabled.
