...
\h{NumPy (Numerical Python) for Architectural/Structural Computing}
...
\end{tcolorbox}
\begin{tcolorbox}[
    title=Visualizing Arrays as Buildings - cont'd,
    colframe=blue,
    colback = white,
    arc=2mm
    breakable
...
\begin{tcolorbox}[
    title=How Filtering Works (Visual),
    colframe=orange,
    colback = white,
    arc=2mm
]
...
(areas >= 35) & (areas <= 50)
     |              |
  [F,T,T,F,T,T,T,T] & [T,T,F,T,T,F,T,T]
         |
  [F,T,F,F,T,F,T,T] --> Result: [45.5, 48.0, 42.0, 38.5]
...
+-----+-----+-----+-----+-----+   +-----+-----+-----+
Original unchanged! /              Only copy changed
...
/ (3, 4) with scalar     --> scalar expands to (3, 4)
/ (3, 4) with (3, 1)     --> (3, 1) expands to (3, 4)
/ (3, 4) with (1, 4)     --> (1, 4) expands to (3, 4)
X (3, 4) with (3, 3)     --> ERROR! Incompatible shapes

\end{lstlisting}
\end{tcolorbox}

\code{c14_broadcasting_detailed}{python}{
    import numpy as np
    
    # Example 1: Unit conversion for all rooms at once
    areas_m2 = np.array([45.0, 52.0, 48.0, 55.0, 38.0])
    
    # Convert ALL areas to square feet (1 m^2 = 10.764 ft^2)
    areas_ft2 = areas_m2 * 10.764  # Multiply every element by 10.764
    print("Areas in m^2:", areas_m2)
    print("Areas in ft^2:", areas_ft2)
    
    # Example 2: Apply safety factor to all loads
    loads_kN = np.array([12.0, 15.0, 13.5, 14.0])
    safety_factor = 1.5
    design_loads = loads_kN * safety_factor
    print("\nDesign loads:", design_loads)
    
    # Example 3: Room dimensions to areas (arrays with arrays)
    widths = np.array([3.0, 3.5, 4.0, 3.8])  # meters
    lengths = np.array([8.0, 7.5, 6.0, 8.0])  # meters
    areas = widths * lengths  # Multiply corresponding elements
    print("\nRoom areas:", areas)
    
    # Example 4: Apply different factors to different floors
    # Building with 3 floors, 4 units each
    base_areas = np.array([
        [45, 52, 48, 55],
        [45, 52, 48, 55],
        [45, 52, 48, 55]
    ...
    # Different reduction factors per floor (for net area calculation)
    floor_factors = np.array([[0.85],  # Ground floor
                              [0.87],  # Floor 1
                              [0.89]]) # Floor 2
    
    net_areas = base_areas * floor_factors  # Broadcasting!
    print("\nNet areas after reduction factors:")
    print(net_areas)
    
    # Outcome:
    # Areas in m^2: [45. 52. 48. 55. 38.]
    # Areas in ft^2: [484.38 559.728 516.672 592.02  408.832]
    # 
    # Design loads: [18.  22.5 20.25 21. ]
    # 
    # Room areas: [24.  26.25 24.  30.4 ]
    # 
    # Net areas after reduction factors:
    # [[38.25 44.2  40.8  46.75]
    #  [39.15 45.24 41.76 47.85]
    #  [40.05 46.28 42.72 48.95]]
}{Apply calculations to entire arrays at once}
...
\begin{tcolorbox}[
    title=Trigonometry in Architecture (Visual),
    colframe=teal,
    colback = white,
    arc=2mm
]
