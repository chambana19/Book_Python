...
Matplotlib is an external library. If you are using Spyder, it is already
...
\begin{lstlisting}[
    language=bash,
    style=block,
    caption={Install Matplotlib from a terminal, not inside a Python file},
    label={c13_install_matplotlib},
    captionpos=b
]
...
These six are independent previews of techniques this chapter does not build
toward in depth -- later chapters and the building-performance dashboard
ahead reuse the same Figure/Axes pattern on different questions and data, not
these exact six panels.
...
fig, ax = plt.subplots(layout="constrained")
ax.barh(materials, quantities_m3, color="tab:blue")
ax.set(
...
\textbf{Hints}
\begin{itemize}
    \item \textbf{Hint 1}: Create a Figure with two Axes side by side.
