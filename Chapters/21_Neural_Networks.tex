...
gradient descent (\booklink{ch:19}{Chapter 19}) and whose architecture is
selected by cross-validation (\booklink{ch:20}{Chapter 20}). What is new is that
...
\booklink{ch:19}{Chapter 19}, and the classification workflow of
\booklink{ch:20}{Chapter 20}: stratified splits, pipelines, cross-validation,
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch21_neuron_anatomy.pdf}
...
logistic regression from \booklink{ch:20}{Chapter 20}. A network is what you get
...
\begin{lstlisting}[language=text, style=block, caption={Depth without nonlinearity is an illusion}, label={c21_collapse_output}, captionpos=b]
Identical: True
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch21_activations.pdf}
...
The right panel of Figure~\ref{fig:ch21-activations} matters more than the left.
...
\hh{Prepare the Chapter 20 data for a network}

The dataset is the comfort-complaint generator from
Listing~\ref{c20_data}, unchanged, so every result in this chapter can be
...
\begin{lstlisting}[language=text, style=block, caption={Every feature now has mean zero and unit variance}, label={c21_data_output}, captionpos=b]
...
still decreases a little. \booklink{ch:19}{Chapter 19} gave the remedy: compare
...
\booklink{ch:19}{Chapter 19}: compute the gradient, take a step opposite it,
...
\begin{lstlisting}[language=text, style=block, caption={400 epochs of full-batch descent}, label={c21_train_output}, captionpos=b]
Loss: 0.7663 -> 0.3554
\end{lstlisting}

\bookfigure[0.98\textwidth]{Figures/Generated/ch21_network_training.pdf}
...
The right panel of Figure~\ref{fig:ch21-training} is the payoff. Complaints rise
...
\booklink{ch:20}{Chapter 20} while this network reaches 0.903 on the same test
rows.
...
learning rate is too large, exactly as in \booklink{ch:19}{Chapter 19}. A
...
In \booklink{ch:20}{Chapter 20} scaling improved logistic regression's
...
\begin{lstlisting}[
    language=python,
    style=block,
    caption={A network as one more estimator in the Chapter 20 workflow},
    label={c21_mlp},
    captionpos=b
]
...
\begin{lstlisting}[language=text, style=block, caption={A 129-parameter network}, label={c21_mlp_output}, captionpos=b]
Iterations run: 546
...
several seeds -- the same discipline \booklink{ch:19}{Chapter 19} demanded of
metaheuristics.
\end{ExplainBox}

\hh{Width and regularization control capacity}

\bookfigure[0.98\textwidth]{Figures/Generated/ch21_network_capacity.pdf}
...
\booklink{ch:20}{Chapter 20}, and it carries the same lesson: the training score
...
\hh{Applied example: a network in the Chapter 20 workflow}
\label{sec:c21-application}

The runnable program \c{examples/c21_neural_network_comfort.py} trains the
hand-written network with a verified gradient, then adds \c{MLPClassifier} as a
fourth candidate in the Chapter 20 comparison and exports the loss curve and
...
\c{predict_proba()}, so it slots into the Chapter 20 procedure without special
...
In Listing~\ref{c21_forward}, identify the shape of every array for 675 training
...
\begin{lstlisting}[
    ...
    label={c21_solution_early_stopping},
    captionpos=b
]
import numpy as np
...
\begin{tcolorbox}[title=Part VI summary,colframe=orange,colback=white,arc=2mm]
