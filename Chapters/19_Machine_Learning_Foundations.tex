...
DataFrames, plots, and algorithm contracts from \booklink{ch:17}{Chapter 17}.
This chapter uses reproducible
...
scikit-learn is not included with Spyder's standalone installer. If you are
using Spyder, follow \booklink{note:spyder-add-package}{the three steps from
Chapter 1} with \c{scikit-learn} as the package name. If you are using
Python 3 and VS Code, install it in a terminal:

\begin{lstlisting}[
    language=bash,
    style=block,
    caption={Install scikit-learn from a terminal},
    label={c19_install_sklearn},
    captionpos=b
]
...
\bookfigure[0.98\textwidth]{Figures/Generated/ch19_ml_workflow.pdf}
...
The features and target used from here on are the synthetic building data
defined in Listing~\ref{c19_dataset} below; the generator is repeated in
Section~\ref{sec:c19-application} so that section runs independently.

\begin{lstlisting}[
    language=python,
    style=block,
    caption={Create the features and target used throughout this chapter},
    label={c19_dataset},
    captionpos=b
]
import numpy as np
...
def make_building_data(seed=42, sample_count=240):
    ...
    # mean 0, standard deviation 10 -- random noise added to each value
...
\begin{lstlisting}[language=text, style=block, caption={240 samples described by four features}, label={c19_dataset_output}, captionpos=b]
features: (240, 4) target: (240,)
\end{lstlisting}
...
print("training rows:", len(y_train), "test rows:", len(y_test))
\end{lstlisting}

Expected output:

\begin{lstlisting}[language=text, style=block, caption={One quarter of the rows are set aside}, label={c19_split_output}, captionpos=b]
...
pattern can indicate nonlinearity, changing variance (the residuals' spread
widening or narrowing across the range), omitted features, data errors, or
distinct populations.

\bookfigure[0.96\textwidth]{Figures/Generated/ch19_regression_diagnostics.pdf}
...
{fig:ch19-diagnostics}

The synthetic example performs well because its target was deliberately
generated from a linear relationship with added random noise. Real building data contain sensor
errors, schedules, weather dependence, interactions, and operational changes.

\hh{Applied example: building-energy regression}
\label{sec:c19-application}

The runnable program \c{examples/c19_building_energy_ml.py} generates documented
...
# A binary label derived from the same rows: is energy use above the median?
high_use = (y > y.median()).astype(int)
high_use_train, high_use_test = train_test_split(
    high_use,
    test_size=0.25,
    random_state=42,
)

# Classification: learn labeled categories. floor_area_m2 spans hundreds
# while glazing_ratio spans less than one, so scale first, inside a Pipeline
# fitted only on the training rows.
classifier = Pipeline([
    ("scale", StandardScaler()),
    ("model", LogisticRegression()),
])
classifier.fit(X_train, high_use_train)
predicted_labels = classifier.predict(X_test)
print(confusion_matrix(high_use_test, predicted_labels))

# Clustering: discover feature-space groups without target labels. KMeans
# measures distance directly, so the same scale mismatch would otherwise let
# floor_area_m2 alone decide the groups.
...
\begin{lstlisting}[language=text, style=block, caption={The same estimator interface, three different tasks}, label={c19_other_tasks_output}, captionpos=b]
[[28  2]
 [ 1 29]]
cluster sizes: [79 70 91]
\end{lstlisting}

Reusing \c{random_state=42} and the same row count makes this split select
exactly the same rows as the earlier one, so \c{high_use_train} lines up with
\c{X_train}. Deriving a label by thresholding the target is a teaching
convenience, not a modeling recommendation: it discards information and invents
a boundary the data never had.

Scaling here is not optional. Without it, \c{LogisticRegression} needs far
more iterations to converge because the solver takes tiny, slow steps in the
narrow \c{glazing_ratio} direction and huge ones in the wide
\c{floor_area_m2} direction, and \c{KMeans} would group rows almost entirely
by \c{floor_area_m2}, since its raw numeric range dwarfs every other column.
Scaling both estimators removes that distortion, and with it the earlier
convergence warning that would otherwise call for raising \c{max_iter}.
...
\booklink{ch:20}{Chapter 20} opens the solvers that fit such models, and
\booklink{ch:21}{Chapter 21} extends this workflow to classification, where
