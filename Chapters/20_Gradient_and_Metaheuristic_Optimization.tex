...
\c{(0.65, 0.60)}. You should obtain roughly 93.9, 89.2, and 111.7. Print the
...
\begin{itemize}
    ...
    \item The same energy term also includes $60(d-0.45)^2$, which contributes
          $120(d-0.45)$ to the depth slope.
    \item The interaction $-70rd$ contributes $-70d$ to the ratio slope and
          $-70r$ to the depth slope.
    \item The shading cost $40d^2$ contributes $80d$ to the depth slope.
...
\c{facade_gradient} or a finite-difference estimator. The \c{cost} parameter is
accepted here but not used yet -- it is reserved for the exercise ahead, which
asks you to also track the cost at each step.
\end{SyntaxBox}
...
noise, catalog components come in fixed increments, and fitted response
